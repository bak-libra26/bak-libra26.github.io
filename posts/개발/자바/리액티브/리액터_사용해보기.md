...
즉, 연산자는 `map()`, `filter()`와 같은 메서드를 의미하며, **위쪽에서 올라오는 데이터를 받아 가공한 뒤, 그 결과를 다시 아래쪽으로 전달하는 중간 처리자 역할을 하는 메서드*** 니다.
...
다음은 `IntelliJ IDEA` 에서 `map` 연산자의 마블 다이어그램 예시입니다.
