...
| 값                       | 설명                            | 브라우저 지원  |
| ----------------------- | ----------------------------- | -------- |
| noarchive               | 검색 엔진이 캐시본을 만들지 않도록 함         | 구글, Bing |
| nocache                 | noarchive와 동일                 | 일부 엔진    |
| nosnippet               | 검색 결과에서 페이지 설명(snippet) 표시 금지 | 구글, Bing |
| noimageindex            | 이 페이지의 이미지를 검색 결과에 표시 금지      | 구글       |
| max-snippet:-1          | 스니펫 길이 제한 (문자 단위)             | 구글       |
| max-image-preview:large | 이미지 미리보기 크기 제한                | 구글       |
| max-video-preview:-1    | 비디오 미리보기 길이 제한 (초 단위)         | 구글       |
