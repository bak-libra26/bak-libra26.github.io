...
---
# Jenkins - Publish Over SSH 서버 테스트 접속 실패

Jenkins에서 `Publish Over SSH` 플러그인을 사용해 서버를 추가하고, `Test Configuration`으로 접속 테스트를 수행할 때 발생할 수 있는 대표 에러 메시지와 해결 방법을 정리합니다.
...
- **해결 방법:**
	...
	- Jenkins의 서버 설정에서 Host/Port 값 재확인
...
- **해결 방법:**
1. 배포할 서버의 `/home/${user}/.ssh/authorized_keys`에 Jenkins의 공개키 추가
2. 권한/소유자 설정:
	- `chmod 600 /home/${user}/.ssh/authorized_keys`
	- `chmod 700 /home/${user}/.ssh`
	- `chown -R ${user}:${user} /home/${user}/.ssh`
3. Jenkins 설정에서 올바른 Private key(내용/경로)와 로그인 사용자명 사용
