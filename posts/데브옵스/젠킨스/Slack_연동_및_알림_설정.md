...
---
# Jenkins Slack 플러그인 연동 및 알림 설정

## Slack에 Jenkins CI 플러그인 추가

- Slack에서 `더보기` → `앱` → `Jenkins CI` 추가
    

## Jenkins에서 Slack 플러그인 설정값 확인
...
## Jenkins에 Slack 설정 추가

- `Manage Jenkins` → `System` → `Slack` 메뉴로 이동
...
## Jenkinsfile에 Slack 알림 스크립트 추가

- `Jenkinsfile`에 빌드 성공/실패에 따라 슬랙 알림 전송 스크립트 추가
