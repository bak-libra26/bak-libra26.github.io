...
Jenkins와 GitHub를 연동하면 코드 변경(push, PR 등) 시 자동으로 파이프라인이 실행되어 CI/CD를 실현할 수 있음.  

이때 `Webhook` 을 활용하면 개발자가 직접 Jenkins에서 빌드를 수동으로 실행하지 않아도, GitHub 이벤트가 발생할 때마다 파이프라인이 자동으로 트리거됨.


> - **Webhook**: GitHub에서 특정 이벤트(예: push, PR)가 발생하면 Jenkins에 HTTP POST 요청을 보내는 기능
> - **Jenkins Multibranch Pipeline**: 다양한 브랜치별로 자동으로 파이프라인 생성 및 관리 가능
...
Github 이벤트를 Jenkins에서 처리하려면 웹훅 IP를 방화벽에 허용해야 합니다.

Github에서 특정 레포지토리나 조직(Organization)에서 발생하는 이벤트를 Jenkins에서 자동으로 처리하려면, Github가 이벤트를 웹훅(Webhook) 형식으로 Jenkins 서버의 URL로 전달한다. 이때, Jenkins 서버가 외부에서 오는 Github 웹훅 요청을 정상적으로 수신하려면, Github 웹훅용 IP 대역을 방화벽에서 허용해야 한다.
...
Jenkins가 GitHub Webhook을 수신하여 자동으로 빌드를 실행하려면 **Generic Webhook Trigger** 플러그인을 설치한다.  
설치는 Jenkins의 `Manage Jenkins` → `Manage Plugins` → `Available Plugins`에서 **Generic Webhook Trigger**를 검색해 설치한다.  
이 플러그인이 없으면 파이프라인에서 웹훅 관련 설정이 보이지 않으므로, 반드시 먼저 설치해야 한다.

> 필수 플러그인: [Generic Webhook Trigger](https://plugins.jenkins.io/generic-webhook-trigger/) 설치

---

#### 2. Pipeline: GitHub

Jenkins와 GitHub 저장소 연동을 위해 **Pipeline: GitHub** 플러그인을 설치한다.  
...
- 확인사항
	...
	2. Jenkinsfile 의 경로
		...
		Jenkinsfile 정상적으로 읽었는지 여부는 `${JENKINS_HOME}/workspace/${pipeline_name}_${branch_name}/jenkins/Jenkinsfile` 이나 로그를 통해 `Jenkins` 내부 파일을 확인해본다.

	3. GitHub 방화벽 및 깃허브 정상 연동 여부
		
		만약 GitHub 의 이벤트를 감지하여 빌드가 실행되었다면 연동에는 문제가 없다.
		
	4. [Plugin: Declarative Pipeline](https://plugins.jenkins.io/pipeline-model-definition) 플러그인의 설치 여부를 확인한다
